...
__device__ int* gravedad_horizontal_TNT(int* dev_R, int f, int c, int fila, int pos, int pos2, int pos3, int pos4, int pos5, int pos6, int n, int dificultad, curandState* estado) {
...
__device__ int* gravedad_vertical_TNT(int* dev_R, int f, int c, int fila, int pos, int pos2, int pos3, int pos4, int pos5, int pos6, int n, int dificultad, curandState* estado) {
...
__global__ void ponerTNT(int* dev_R, int c, int f, int size, int fila, int columna, int* TNT, int n, int dificultad, curandState* estado) {
	...
	//printf("pos: %d %d", pos, posUsuario);
	//comprueba que se esta en la pos en la que esta interactuando el usuario
	if (pos == posUsuario) {
		//se comprueba cada una de las posibles combinaciones
		//printf("Pos: %d %d  %d %d", pos - (c * 4), pos - (c * 3), pos - (c * 2), pos - c);
		//Comprueba en horizontal
		//comprueba que tengan el mismo valro y que esten en la misma fila
		if (dev_R[pos + 1] == valor && dev_R[pos + 2] == valor && dev_R[pos + 3] == valor && dev_R[pos + 4] == valor && dev_R[pos + 5] == valor && (pos + 1) / c == filaUsuario && (pos + 2) / c == filaUsuario && (pos + 3) / c == filaUsuario && (pos + 4) / c == filaUsuario && (pos + 5) / c == filaUsuario) {
			printf("Posicion que va a poner el TNT: %d %d", fil, col);
			dev_R = gravedad_horizontal_TNT(dev_R, f, c, fila, pos, pos + 1, pos + 2, pos + 3, pos + 4, pos + 5, n, dificultad, estado);
			*TNT += 1;
		}
		if (dev_R[pos - 1] == valor && dev_R[pos + 1] == valor && dev_R[pos + 2] == valor && dev_R[pos + 3] == valor && dev_R[pos + 4] == valor && (pos - 1) / c == filaUsuario && (pos + 1) / c == filaUsuario && (pos + 2) / c == filaUsuario && (pos + 3) / c == filaUsuario && (pos + 4) / c == filaUsuario) {
			printf("Posicion que va a poner el TNT: %d %d", fil, col);
			dev_R = gravedad_horizontal_TNT(dev_R, f, c, fila, pos, pos - 1, pos + 1, pos + 2, pos + 3, pos + 4, n, dificultad, estado);
			*TNT += 1;
		}
		if (dev_R[pos - 2] == valor && dev_R[pos - 1] == valor && dev_R[pos + 1] == valor && dev_R[pos + 2] == valor && dev_R[pos + 3] == valor && (pos - 2) / c == filaUsuario && (pos - 1) / c == filaUsuario && (pos + 1) / c == filaUsuario && (pos + 2) / c == filaUsuario && (pos + 3) / c == filaUsuario) {
			printf("Posicion que va a poner el TNT: %d %d", fil, col);
			dev_R = gravedad_horizontal_TNT(dev_R, f, c, fila, pos, pos - 2, pos - 1, pos + 1, pos + 2, pos + 3, n, dificultad, estado);
			*TNT += 1;
		}
		if (dev_R[pos - 3] == valor && dev_R[pos - 2] == valor && dev_R[pos - 1] == valor && dev_R[pos + 1] == valor && dev_R[pos + 2] == valor && (pos - 3) / c == filaUsuario && (pos - 2) / c == filaUsuario && (pos - 1) / c == filaUsuario && (pos + 1) / c == filaUsuario && (pos + 2) / c == filaUsuario) {
			printf("Posicion que va a poner el TNT: %d %d", fil, col);
			dev_R = gravedad_horizontal_TNT(dev_R, f, c, fila, pos, pos - 3, pos - 2, pos - 1, pos + 1, pos + 2, n, dificultad, estado);
			*TNT += 1;
		}
		if (dev_R[pos - 4] == valor && dev_R[pos - 3] == valor && dev_R[pos - 2] == valor && dev_R[pos - 1] == valor && dev_R[pos + 1] == valor && (pos - 4) / c == filaUsuario && (pos - 3) / c == filaUsuario && (pos - 2) / c == filaUsuario && (pos - 1) / c == filaUsuario && (pos + 1) / c == filaUsuario) {
			printf("Posicion que va a poner el TNT: %d %d", fil, col);
			dev_R = gravedad_horizontal_TNT(dev_R, f, c, fila, pos, pos - 4, pos - 3, pos - 2, pos - 1, pos + 1, n, dificultad, estado);
			*TNT += 1;
		}
		if (dev_R[pos - 5] == valor && dev_R[pos - 4] == valor && dev_R[pos - 3] == valor && dev_R[pos - 2] == valor && dev_R[pos - 1] == valor && (pos - 5) / c == filaUsuario && (pos - 4) / c == filaUsuario && (pos - 3) / c == filaUsuario && (pos - 2) / c == filaUsuario && (pos - 1) / c == filaUsuario) {
			printf("Posicion que va a poner el TNT: %d %d", fil, col);
			dev_R = gravedad_horizontal_TNT(dev_R, f, c, fila, pos, pos - 5, pos - 4, pos - 3, pos - 2, pos - 1, n, dificultad, estado);
			*TNT += 1;
		}

		//Comprueba en vertical
		//comprueba que tengan el mismo valro y que esten en la misma fila
		if (dev_R[pos + c] == valor && dev_R[pos + (c * 2)] == valor && dev_R[pos + (c * 3)] == valor && dev_R[pos + (c * 4)] == valor && dev_R[pos + (c * 5)] == valor) {
			printf("Posicion que va a poner el TNT6: %d %d", fil, col);
			dev_R = gravedad_vertical_TNT(dev_R, f, c, fila, pos, pos + c, pos + (c * 2), pos + (c * 3), pos + (c * 4), pos + (c * 5), n, dificultad, estado);
			*TNT += 1;
		}
		else if (dev_R[pos - c] == valor && dev_R[pos + c] == valor && dev_R[pos + (c * 2)] == valor && dev_R[pos + (c * 3)] == valor && dev_R[pos + (c * 4)] == valor) {
			printf("Posicion que va a poner el TNT7: %d %d", fil, col);
			dev_R = gravedad_vertical_TNT(dev_R, f, c, fila, pos - c, pos, pos + c, pos + (c * 2), pos + (c * 3), pos + (c * 4), n, dificultad, estado);
			*TNT += 1;
		}
		else if (dev_R[pos - (c * 2)] == valor && dev_R[pos - c] == valor && dev_R[pos + c] == valor && dev_R[pos + (c * 2)] == valor && dev_R[pos + (c * 3)] == valor) {
			printf("Posicion que va a poner el TNT8: %d %d", fil, col);
			dev_R = gravedad_vertical_TNT(dev_R, f, c, fila, pos - (c * 2), pos - c, pos, pos + c, pos + (c * 2), pos + (c * 3), n, dificultad, estado);
			*TNT += 1;
		}
		else if (dev_R[pos - (c * 3)] == valor && dev_R[pos - (c * 2)] == valor && dev_R[pos - c] == valor && dev_R[pos + c] == valor && dev_R[pos + (c * 2)] == valor) {
			printf("Posicion que va a poner el TNT9: %d %d", fil, col);
			dev_R = gravedad_vertical_TNT(dev_R, f, c, fila, pos - (c * 3), pos - (c * 2), pos - c, pos, pos + c, pos + (c * 2), n, dificultad, estado);
			*TNT += 1;
		}
		else if (dev_R[pos - (c * 4)] == valor && dev_R[pos - (c * 3)] == valor && dev_R[pos - (c * 2)] == valor && dev_R[pos - c] == valor && dev_R[pos + (c * 1)] == valor) {
			printf("Posicion que va a poner el TNT10: %d %d", fil, col);
			dev_R = gravedad_vertical_TNT(dev_R, f, c, fila, pos - (c * 4), pos - (c * 3), pos - (c * 2), pos - c, pos, pos + c, n, dificultad, estado);
			*TNT += 1;
		}
		else if (dev_R[pos - (c * 5)] == valor && dev_R[pos - (c * 4)] == valor && dev_R[pos - (c * 3)] == valor && dev_R[pos - (c * 2)] == valor && dev_R[pos - c] == valor) {
			printf("Posicion que va a poner el TNT10: %d %d", fil, col);
			dev_R = gravedad_vertical_TNT(dev_R, f, c, fila, pos - (c * 5), pos - (c * 4), pos - (c * 3), pos - (c * 2), pos - c, pos, n, dificultad, estado);
			*TNT += 1;
		}
	}
}
...
__device__ int* gravedad_vertical_bomba(int* dev_R, int f, int c, int fila, int pos, int pos2, int pos3, int pos4, int pos5, int n, int dificultad, curandState* estado) {
	//se obtiene cuando comienza los 4 numeros adyancentes
	int fila_ini = pos / c;
	//se obtiene en que fila termina los cuatro repetidos
	int fila_ult = pos5 / c;
...
__device__ int* gravedad_horizontal_bomba(int* dev_R, int f, int c, int fila, int pos, int pos2, int pos3, int pos4, int pos5, int n, int dificultad, curandState* estado) {
	//fila actual de la posicion
	//int fila_actual = pos/c;
...
__global__ void ponerBomba(int* dev_R, int c, int f, int size, int fila, int columna, int* bomba, int n, int dificultad, curandState* estado) {
	...
	if (pos == posUsuario) {
		...
		//Comprueba en horizontal
		//comprueba que tengan el mismo valro y que esten en la misma fila
		if (dev_R[pos + 1] == valor && dev_R[pos + 2] == valor && dev_R[pos + 3] == valor && dev_R[pos + 4] == valor && (pos + 1) / c == filaUsuario && (pos + 2) / c == filaUsuario && (pos + 3) / c == filaUsuario && (pos + 4) / c == filaUsuario) {
			printf("Posicion que va a poner la B: %d %d", fil, col);
			//printf("\nHola1\n");
			dev_R = gravedad_horizontal_bomba(dev_R, f, c, fila, pos, pos + 1, pos + 2, pos + 3, pos + 4, n, dificultad, estado);
			*bomba += 1;
		}if (dev_R[pos - 1] == valor && dev_R[pos + 1] == valor && dev_R[pos + 2] == valor && dev_R[pos + 3] == valor && (pos - 1) / c == filaUsuario && (pos + 1) / c == filaUsuario && (pos + 2) / c == filaUsuario && (pos + 3) / c == filaUsuario) {
			printf("Posicion que va a poner la B: %d %d", fil, col);
			//printf("\nHola2\n");
			dev_R = gravedad_horizontal_bomba(dev_R, f, c, fila, pos, pos - 1, pos + 1, pos + 2, pos + 3, n, dificultad, estado);
			*bomba += 1;
		}
		else if (dev_R[pos - 2] == valor && dev_R[pos - 1] == valor && dev_R[pos + 1] == valor && dev_R[pos + 2] == valor && (pos - 2) / c == filaUsuario && (pos - 1) / c == filaUsuario && (pos + 1) / c == filaUsuario && (pos + 2) / c == filaUsuario) {
			//printf("\nHola3\n");
			printf("Posicion que va a poner la B: %d %d", fil, col);
			dev_R = gravedad_horizontal_bomba(dev_R, f, c, fila, pos, pos - 2, pos - 1, pos + 1, pos + 2, n, dificultad, estado);
			*bomba += 1;
		}
		else if (dev_R[pos - 3] == valor && dev_R[pos - 2] == valor && dev_R[pos - 1] == valor && dev_R[pos + 1] == valor && (pos - 3) / c == filaUsuario && (pos - 2) / c == filaUsuario && (pos + 1) / c == filaUsuario && (pos - 1) / c == filaUsuario) {
			//printf("\nHola4\n");
			printf("Posicion que va a poner la B: %d %d", fil, col);
			dev_R = gravedad_horizontal_bomba(dev_R, f, c, fila, pos, pos - 3, pos - 2, pos - 1, pos + 1, n, dificultad, estado);
			*bomba += 1;
		}
		else if (dev_R[pos - 4] == valor && dev_R[pos - 3] == valor && dev_R[pos - 2] == valor && dev_R[pos - 1] == valor && (pos - 4) / c == filaUsuario && (pos - 3) / c == filaUsuario && (pos - 2) / c == filaUsuario && (pos - 1) / c == filaUsuario) {
			//printf("\nHola41\n");
			printf("Posicion que va a poner la B: %d %d", fil, col);
			dev_R = gravedad_horizontal_bomba(dev_R, f, c, fila, pos, pos - 4, pos - 3, pos - 2, pos - 1, n, dificultad, estado);
			*bomba += 1;
		}

		//En vertical
		//compruba que los 3 elementos de arriba tienen el mismo valor
		if (dev_R[pos + c] == valor && dev_R[pos + (c * 2)] == valor && dev_R[pos + (c * 3)] == valor && dev_R[pos + (c * 4)] == valor && pos + (c * 3 < size)) {
			printf("Posicion que va a poner la B: %d %d", fil, col);
			//printf("\nHola5\n");
			dev_R = gravedad_vertical_bomba(dev_R, f, c, fila, pos, pos + c, pos + (c * 2), pos + (c * 3), pos + (c * 4), n, dificultad, estado);
			*bomba += 1;

		}
		else if (dev_R[pos - c] == valor && dev_R[pos + c] == valor && dev_R[pos + (c * 2)] == valor && dev_R[pos + (c * 3)] == valor) {
			//printf("\nHola6\n");
			printf("Posicion que va a poner la B: %d %d", fil, col);
			dev_R = gravedad_vertical_bomba(dev_R, f, c, fila, pos - c, pos, pos + c, pos + (c * 2), pos + (c * 3), n, dificultad, estado);
			*bomba += 1;
		}
		else if (dev_R[pos - (c * 2)] == valor && dev_R[pos - c] == valor && dev_R[pos + c] == valor && dev_R[pos + (c * 2)] == valor) {
			//printf("\nHola7\n");
			printf("Posicion que va a poner la B: %d %d", fil, col);
			dev_R = gravedad_vertical_bomba(dev_R, f, c, fila, pos - (c * 2), pos - c, pos, pos + c, pos + (c * 2), n, dificultad, estado);
			*bomba += 1;
		}
		else if (dev_R[pos - (c * 3)] == valor && dev_R[pos - (c * 2)] == valor && dev_R[pos - c] == valor && dev_R[pos + c] == valor) {
			//printf("\nHola8\n");
			printf("Posicion que va a poner la B: %d %d", fil, col);
			dev_R = gravedad_vertical_bomba(dev_R, f, c, fila, pos - (c * 3), pos - (c * 2), pos - c, pos, pos + c, n, dificultad, estado);
			*bomba += 1;
		}
		else if (dev_R[pos - (c * 4)] == valor && dev_R[pos - (c * 3)] == valor && dev_R[pos - (c * 2)] == valor && dev_R[pos - c] == valor) {
			//printf("\nHola8\n");
			printf("Posicion que va a poner la B: %d %d", fil, col);
			dev_R = gravedad_vertical_bomba(dev_R, f, c, fila, pos - (c * 4), pos - (c * 3), pos - (c * 2), pos - c, pos, n, dificultad, estado);
			*bomba += 1;
		}
	}
...
__global__ void explotarBomba(int* dev_R, int c, int f, int size, int fila, int columna, int n, int* explosion, int dificultad, curandState* estado) {
	...
	if (pos == posUsuario && dev_R[pos] == -1) {
		...
		if (f_c == 0) {
			//printf("y se genera una nueva fila y caen las demas: \n");
			...
			//printf("y se genera una nueva columna: \n");
			//explota la columna
...
__global__ void comprobarPares(int* dev_R, int c, int f, int size, int fila, int columna, int* par, int dificultad, curandState* estado) {
	...
	// IIIIIIIIIIIIII - COMPRUEBA DOBLES DERECHA
	if (dev_R[pos] == dev_R[pos + 1] && (pos + 1) % c != 0 && pos == posUsuario) {
		//GRAVEDAD(Izquierda)-> Siempre se generan aleatorios los items correspondientes a la f0 y f1, el resto se cogen por gravedad
		int nuevo_f0_izq = 0; //Valor Declarativo
		int nuevo_f0_der = 0; //Valor Declarativo
		//printf("\nSe añaden los valores: ");
		if (dificultad == 1) { //Coge aleatorios los nuevos valores de la fila 0 y fila 1
			nuevo_f0_izq = 1 + (int)(curand_uniform(estado) * 4);
			nuevo_f0_der = 1 + (int)(curand_uniform(estado) * 4);
		}
		else {
			nuevo_f0_izq = 1 + (int)(curand_uniform(estado) * 6);
			nuevo_f0_der = 1 + (int)(curand_uniform(estado) * 6);
		}

		if (fila == 0) {
			dev_R[pos] = nuevo_f0_der;
			dev_R[pos + 1] = nuevo_f0_izq;
			//printf("%d ", dev_R[pos]);
			//printf("%d ", dev_R[pos + 1]);
		}
		else {
			int kAux = 1;
			int kFinal = 0;
			for (int k = 0; k < fila; k++) {
				...
				dev_R[(pos - (c * k)) + 1] = dev_R[(pos - (c * kAux)) + 1];
				kAux++;

				//printf("%d ", dev_R[pos - (c * k)]);
				//printf("%d ", dev_R[(pos - (c * k)) + 1]);
				kFinal = k;
			}
			kFinal++;
			dev_R[pos - (c * kFinal)] = nuevo_f0_der; //Rellena valor de la fila 0
			//printf("%d ", dev_R[pos - (c * kFinal)]);
			dev_R[(pos - (c * kFinal)) + 1] = nuevo_f0_izq; //Rellena valor de la fila 0
			//printf("%d\n", dev_R[(pos - (c * kFinal)) + 1]);
		}
		*par += 1;
		*par += 1;
	}
	// IIIIIIIIIIIIII - COMPRUEBA DOBLES IZQUIERDA
	//else if (dev_R[pos] == dev_R[pos - 1] && (pos - 1) % c != 0 && pos == posUsuario && (pos - 1) >= (fila * posUsuario)) {    XXXXXXXXXX - ESTO DABA MAL, SOLO SALIE BIEN SI SE COGIA LA LINEA 0
	else if (dev_R[pos] == dev_R[pos - 1] && (pos - 1) % c != 0 && pos == posUsuario && (pos - 1) <= (fila * posUsuario)) {
		//GRAVEDAD(Izquierda)-> Siempre se generan aleatorios los items correspondientes a la f0 y f1, el resto se cogen por gravedad
		int nuevo_f0_izq = 0; //Valor Declarativo
		int nuevo_f0_der = 0; //Valor Declarativo
		//printf("\nSe añaden los valores: ");
	...
	// IIIIIIIIIIIIII - COMPRUEBA DOBLES ARRIBA
	else if (dev_R[pos] == dev_R[pos - c] && pos - c > 0 && pos == posUsuario) {
		//GRAVEDAD(Arriba)-> Siempre se generan nuevos (aleatorios) los items de (fila=0 y fila=1) el resto de items añaden por gravedad en "fila-1" bucles.
	...
	// IIIIIIIIIIIIII - COMPRUEBA DOBLES DEBAJO
	else if (dev_R[pos] == dev_R[pos + c] && pos < size && pos == posUsuario) {
		//GRAVEDAD(Abajo)-> Siempre se generan nuevos (aleatorios) los items de (fila=0 y fila=1) el resto de items añaden por gravedad en "fila-1" bucles.
		int nuevo_f0 = 0; //Valor Declarativo
		int nuevo_f1 = 0; //Valor Declarativo
		//printf("\nSe añaden los valores: ");
		if (dificultad == 1) { //Coge aleatorios los nuevos valores de la fila 0 y fila 1
...
int main(int argc, char** argv) {
	...
	blockSize = (rand() % (minimo_global / 2)) + 1;

	int minimo_hilos_matriz = minimo_filas_columnas;
	if (minimo_hilos <= minimo_hilos_matriz) {
		minimo_hilos_matriz = minimo_hilos;
	}

	tamano = (rand() % (minimo_hilos_matriz / 2)) + 1;

	printf("Bloques Elegidos: %d\n", blockSize);
	printf("Hilos Elegidos: %d\n", tamano);
	...
	*/
	...
	dim3 ThreadsBloque((tamano + blockSize - 1) / blockSize, (tamano + blockSize - 1) / blockSize);
